...
function normalizeType(rawType = '') {
  ...
  if (upper.includes('SELL') || upper.includes('JUAL') || upper.includes('PENJUALAN')) return 'SELL';
  if (upper.includes('BUY') || upper.includes('BELI') || upper.includes('PEMBELIAN')) return 'BUY';
  if (upper.includes('DEPOSIT') || upper.includes('SETOR')) return 'DEPOSIT';
  if (upper.includes('WITHDRAW') || upper.includes('TARIK')) return 'WITHDRAWAL';
  if (upper.includes('FEE') || upper.includes('BIAYA')) return 'FEE';
  return 'BUY';
}
...
1. "type": Must be one of: "BUY", "SELL", "DEPOSIT", "WITHDRAWAL", "FEE".
   - "Spot Buy (HYPE)", "Spot Beli (HYPE)", "Beli HYPE" -> "BUY"
   - "Spot Sell (HYPE)", "Spot Jual (HYPE)", "Jual HYPE", "Penjualan" -> "SELL"
   - "Deposit Rupiah (IDR)", "Setor Rupiah" -> "DEPOSIT"
   - "Withdrawal Rupiah", "Tarik Rupiah" -> "WITHDRAWAL"
2. "asset": Cryptocurrency or fiat code (e.g. "HYPE", "BTC", "ETH", "IDR", "USDC").
   - If buying or selling HYPE, asset is "HYPE".
   - If buying or selling BTC, asset is "BTC".
   - For Deposit/Withdrawal Rupiah, asset is "IDR".
3. "quantity": Number of crypto asset units bought or sold (e.g. 1.277149 for HYPE).
   - CRITICAL FOR SELL: "quantity" must be the amount of crypto units being sold (e.g. 1.277149).
   - For IDR deposit/withdrawal, set to the IDR amount (e.g. 9000000).
4. "rate_idr": Price per unit in IDR (e.g. 1406429 or 1620000). If deposit/withdrawal, set to 1.
5. "amount_idr": Gross amount in IDR (e.g. 1800000 or 2068981).
6. "fee_idr": Total fee in IDR (sum of Tax Fee, Triv Fee, QRIS Fee). If Free, 0.
7. "total_received": 
   - For BUY: crypto units received (e.g. 1.277149).
   - For SELL: net IDR received after fees (e.g. 2064844).
   - For DEPOSIT: net IDR received (e.g. 9000000).
