...
async function getMarketPrices() {
  ...
  // 2.5 Fetch Binance USDTIDR bookTicker for live stable crypto USD/IDR rate (Triv/Indodax benchmark)
  try {
    ...
    if (usdtRes.ok) {
      ...
      if (bid > 10000 && ask < 25000) {
        const midRate = (bid + ask) / 2;
        const prev = cachedPrices.USD_IDR || 17758;
        cachedPrices.USD_IDR = Math.round(prev * 0.8 + midRate * 0.2);
      }
    }
  } catch (err) {
    // Keep cached
