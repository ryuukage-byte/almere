...
• /help - Bantuan & tata cara verifikasi
  ...
  if (text.startsWith('/portfolio') || text.startsWith('/status')) {
    try {
      ...
      let holdingsSummary = p.holdings.map(h => `• *${h.name}:* ${h.unitsText} (~$${h.valueUsd.toLocaleString()} · ${h.allocationPct}%)`).join('\n');
...
🕒 *Sinkronisasi:* Real-time
  ...
  if (msg.photo && msg.photo.length > 0) {
    ...
    const notifyMsg = await callTelegramApi('sendMessage', {
      chat_id: chatId,
      text: '🔍 Sedang memindai screenshot Triv dengan AI OCR (Gemini 2.5 Flash)...'
    });
